...
def login_user(payload: LoginRequest) -> dict[str, Any]:
    ...
    # No silent auto-registration here: a login attempt for an email that was
    # never registered must fail, not spin up a fresh ADMIN account. Account
    # creation belongs solely to /auth/register.
    if not user:
        raise HTTPException(
